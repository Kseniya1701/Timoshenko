...
bot = telebot.TeleBot('5242721464:AAEGTV21OggWh_bRXCiqL_wVIqE4Jjlcz5c')  # Создаем экземпляр бота
...
def get_number():
    array_num = []
    req_numb = requests.get('https://calculator888.ru/random-generator/')
    if req_numb.status_code == 200:
        soup = bs4.BeautifulSoup(req_numb.text, "html.parser")
...
def get_facts():
    array_fact = []
    req_f = requests.get('http://megagenerator.ru/fakti')
